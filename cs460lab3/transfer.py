...
class Main(object):
    ...
    def parse_options(self):
        ...
        parser.add_option("-d", "--debug", dest="debug",
                          ...
                          help="debug logging")
        parser.add_option("-s", "--ploss",type="str",dest="ploss",
                          default=" ",
                          help="spec packet loss")
        parser.add_option("-r", "--reno",action="store_true",dest="reno",
                          default=False,
                          help="reno or no reno")

        (options, args) = parser.parse_args()
        self.filename = options.filename
        self.loss = options.loss
        self.debug = options.debug
        self.packets2lose = [] if options.ploss == " " else [int(x) for x in options.ploss.split(",")]
        print(self.packets2lose)
        self.reno = options.reno
    def diff(self):
    ...
    def run(self):
        ...
        c1 = TCP(t1, n1.get_address('n2'), 1, n2.get_address('n1'), 1, a, window=window, drop=drop,reno=self.reno)
