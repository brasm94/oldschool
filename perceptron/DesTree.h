...
// the centroid label.)
class DesTreeLearner : public SupervisedLearner
{
private:
...
public:
	DesTreeLearner(Rand& r)
	: SupervisedLearner(), m_rand(r)
	{
	}

	virtual ~DesTreeLearner()
	{
	}
	...
	virtual void train(Matrix& features, Matrix& labels)
	{
		...
		dn = new DesNode(features,labels);
		dn->makeTree();
		cout <<"out of row = " << dn->getOut(features.row(1)) <<endl;
		// Train it
