...
class RoutingApp(object):
    def __init__(self, node):
        ...
        #     ident=1, ttl=1, protocol='dvrouting',body=self.node.DV,length=100)
        Sim.scheduler.add(delay=0,event=None,handler=self.node.updateTable)
        Sim.scheduler.add(delay=1,event=None,handler=self.node.checkLinks)

    def receive_packet(self, packet, **kwargs):
        logger.debug(str(self.node.hostname) + " Receiving DV from host " + str(packet.ident))
        self.node.neighborsTimer[packet.ident] = 3 ## count down timer.
        self.node.cost2neighbors[packet.ident] = 1
        self.node.DVneighbors[packet.ident] = packet.body
        self.node.updateTable(None)
